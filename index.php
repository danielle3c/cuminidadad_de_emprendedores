...
<div class="container">
    <h2 style="text-align:center;">🔍 Gestión de Emprendedores</h2>
    ...
    if (isset($_GET['buscar']) && !empty($_GET['buscar'])) {
        ...
        // Consulta que une Personas, Emprendedores y Créditos
        $sql = "SELECT p.*, e.idemprendedores, e.rubro, e.tipo_negocio, c.idcreditos, c.saldo_inicial 
                FROM personas p
            ...
            while ($f = mysqli_fetch_assoc($res)) {
                ...
                <div class="result-card">
                    <div style="display:flex; justify-content:space-between; align-items:center;">
                        ...
                        <?php if($f['rubro']) echo '<span class="status-tag">ACTIVO</span>'; ?>
                    </div>
                    ...
                    <div class="grid">
                        ...
                        <div>
                            <?php if ($f['idcreditos']): ?>
                                <p><strong>💳 Crédito ID:</strong> <?php echo $f['idcreditos']; ?></p>
                                <p><strong>📉 Saldo Pendiente:</strong> <span class="debt">$<?php echo number_format($f['saldo_inicial'], 2); ?></span></p>
                            <?php else: ?>
                                <p style="color:gray; font-style:italic;">Sin crédito activo</p>
                            <?php endif; ?>
                        </div>
                    </div>
                    <div style="text-align:right; margin-top:10px;">
                        <a href="cobranzas.php?id_credito=<?php echo $f['idcreditos']; ?>" style="color:#2563eb; text-decoration:none; font-weight:bold;">Registrar Pago →</a>
                    </div>
                </div>
                <?php
            }
        } else {
            echo "<p style='text-align:center; color:gray;'>No se encontraron resultados para '$busqueda'.</p>";
        }
    }
    ?>
</div>
